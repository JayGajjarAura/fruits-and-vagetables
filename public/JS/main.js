...
import { AppDropdownElement } from './dropdown-element.js';
import { fuzzySearch } from './fussy-search.js';

const fuzzySearchBrowsersList = fuzzySearch(BROWSERS_LIST, ['title']);
...
//   console.log('fiterrrrrrrrrrrr',filteredList[0].item.name);
  const cleanFilteredList = filteredList.slice(0, BROWSER_SUGGESTIONS_MAX_SIZE).map(el => el.item.title);
